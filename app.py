...
# PaddleOCR for handwritten prescriptions
try:
...
# Try both possible GenAI client import names 
try:
...
def paddle_extract_text(pil_image):
...
def extract_and_analyze(
    ...
    # PaddleOCR for handwritten text (second priority)
    if (not extracted_text or not extracted_text.strip()) and pil_img is not None:
        try:
            ptxt = paddle_extract_text(pil_img)

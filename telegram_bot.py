...
def clean_text_for_tts(text: str) -> str:
    # Join letter-by-letter spellings like S-U-D-A-Y-O-N-E-R → SUDAYONER
    text = re.sub(
        r'\b([A-Z])(?:-([A-Z])){2,}\b',
...
def generate_audio_summary(doc_type, clean_text):
    return (
...
def process_image(chat_id, file_id, language):
    ...
    try:
        ...
        audio_text = generate_audio_summary(doc_type, clean_analysis)
        audio_bytes = text_to_speech(audio_text, language)
